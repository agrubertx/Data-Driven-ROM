...
def glorot(tensor):
    if tensor is not None:
        stdv = sqrt(6.0 / (tensor.size(-2) + tensor.size(-1)))
...
class GCN2Conv(MessagePassing):
    ...
    def forward(self, x: Tensor, x_0: Tensor, edge_index: Adj,
        ...
        if self.weight2 is None:  ### needed to modify this part
            out = x.add_(x_0)
            # out = torch.addmm(out, out, self.weight1, beta=1. - self.beta,
            #                   alpha=self.beta)
            out = out.view(-1, self.weight1.size(0))
            out = torch.addmm(out, out, self.weight1, beta=1. - self.beta,
                              alpha=self.beta)
            out = out.view(x_0.size(0), -1, self.weight1.size(0))
        else:
